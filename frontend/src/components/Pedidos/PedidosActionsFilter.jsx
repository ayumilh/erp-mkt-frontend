...
const PedidosActionsFilter = ({ setSearchTerm, searchTerm = '', searchColumn, setSearchColumn, setFilteredOrders }) => {
  const handleFilteredOrdersData = (data) => {
    ...
    <div className="bg-primaria-900 dark:bg-dark-primaria-900 hover:shadow-input transition-all ease-in duration-500 rounded-2xl xl:flex w-full min-h-max px-4 py-5 xl:px-8 xl:items-center">
      ...
      <form className="mt-[14px] md:flex lg:justify-start 2xl:justify-center xl:ml-5 w-full">
        <div className='flex w-full items-center'>
          ...
          <div>
            <BtnActionsFilter onConfirm={handleFilteredOrdersData} />
          </div>
        </div>

        <div className='flex w-full items-center'>
          <div className="flex w-3/6 md:w-4/6 lg:w-3/6">
            <select
              title='Coluna de Pesquisa'
              className="dark:bg-neutral-800 w-full dark:text-gray-200 h-9 rounded-l text-xs md:text-sm font-normal py-2 pl-2 pr-4"
              value={searchColumn}
              onChange={(e) => setSearchColumn(e.target.value)}
            >
              <option value="hr_pedido" title='hr do pedido'>Hora do pedido</option>
              <option value="hr_entrega" title='hr de entrega'>Hora da entrega</option>
              <option value="hr_envio" title='hr de envio'>Hora da envio</option>
            </select>
          </div>

          <div className="flex w-4/6 md:w-2/6 lg:w-3/6">
            <input
