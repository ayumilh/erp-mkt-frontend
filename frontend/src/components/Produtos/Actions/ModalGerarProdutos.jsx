...
export default function ModalGerarProdutos({ onClose, idProduct }){
  ...
  return (
    <div className="bg-gray-700 bg-opacity-70 fixed z-50 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full md:max-w-2xl lg:max-w-4xl xl:max-w-5xl">
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <div className="modal-header flex justify-between items-center mb-4">
              <h1 className='text-base text-colorFont-200 font-semibold'>Gerar produtos</h1>
                     ...
                     <table className="table-auto min-w-full mt-4">
                        <thead>
                           <tr>
                              ...
                              <th className="px-4 py-2 md:py-4 text-sm font-semibold text-center">Preço</th>
                              <th className="px-4 py-2 md:py-4 text-sm font-semibold text-center">Status</th>
                           </tr>
                        </thead>
                        <tbody>
                           {product.map((item, index) => (
                              <tr key={index}>
                                 <td className="px-4 py-2 md:py-4">{item.sku}</td>
                                 <td className="break-words md:break-normal px-4 py-2 md:py-4 "><p className="font-medium">{item.titleVariables}</p></td>
                                 <td className="px-4 py-2 md:py-4 text-center">{item.priceVariables}</td>
                                 <td className="px-4 py-2 md:py-4 text-center">{item.statusVariables}</td>
                              </tr>
                           ))}
                        </tbody>
                     </table>
