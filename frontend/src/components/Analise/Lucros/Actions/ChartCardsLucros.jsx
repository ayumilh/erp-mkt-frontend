            ...
            {info.map((item) => (
                <div key={item.titulo} onClick={() => handleItemClick(item)} className={`w-[150px] md:w-40 lg:w-44 h-24 flex flex-col justify-cente shadow-md hover:shadow-lg rounded-md hover:rounded-lg p-3 cursor-pointer transition duration-300 ease-in-out ${selectedItem === item.titulo ? 'bg-segundaria-900' : ''}`}>
                    <div className="flex gap-1 xl:gap-2 items-center justify-start">
                        <span className={`lg:text-lg font-semibold text-left transition duration-300 ease-in-out ${selectedItem === item.titulo ? 'text-gray-200' : ''}`}>0</span>
                        <div className="flex justify-start items-center">
                            <span className={`text-sm font-medium ${selectedItem === item.titulo ? 'text-gray-200' : ''}`}>{item.porcentagem}</span>
                            {item.porcentagem && item.porcentagem.includes('-') ? (
                                <span className="text-red-500 text-xs ml-2">↓</span>
                            ) : (
                                <span className="text-green-500 text-xs md:text-sm ml-2">↑</span>
                            )}
                        </div>
                    </div>
                    <div className="mt-1">
