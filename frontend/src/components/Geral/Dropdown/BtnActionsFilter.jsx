...
const BtnActionsFilter = ({ onConfirm }) => {
  ...
  return (
    <div>
      ...
      {isModalOpen && (
        <div className={`origin-top-center absolute z-50 mt-5 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 transition-transform duration-300 ease-out transform ${isModalOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
          <div ref={modalRef} className="relative bg-white dark:bg-neutral-800 p-4 rounded shadow-lg min-h-max" style={{ width: '620px' }}>
            <div className="absolute top-0 left-5 transform -translate-x-1/2 -translate-y-full">
